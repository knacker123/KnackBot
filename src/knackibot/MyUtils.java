...
public class MyUtils {
   ...
   *   center
   */
  public static double angleBetween(Point2D.Double center, Point2D.Double next,
  ...
  /*
   * Calculates a point p2 on the 2D coordinate system based on - point p1 - distance between p1 and
   * p2 - absolute angle between 12 o'clock and p2
   */
  ...
  /*
   * Calculated absolute angle between line of p1 to p2 and 12 o'clock Note: In Robocode angles are
   * clockwise!
   * 
   * x p2 | / | / |/ x p1
   */
  ...
  /**
   * Possibility to check, whether Point p is inside the battlefield. One can choose to additionally
   * consider a margin. If margin is 0, the rectangle ends with the battlefield, if margin > 0,
   * another, smaller rectangle is considered.
   * 
  ...
  public static boolean isInsideBattleField(Point2D.Double p, double margin) {
    return (isInsideBattleFieldHorizontal( p, margin) && isInsideBattleFieldVertical(p, margin));
  }
  
  private static boolean isInsideBattleFieldHorizontal(Point2D.Double p, double margin) {
    return (p.x > (0 + margin)) && (p.x < (800 + margin));
  }
  
  private static boolean isInsideBattleFieldVertical(Point2D.Double p, double margin) {
    return (p.y > (0 + margin)) && (p.y < (800 + margin));
  }
