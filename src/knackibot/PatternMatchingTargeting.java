...
public class PatternMatchingTargeting implements TargetStrategy {
    ...
    if (enemy.getPosLogSize() > patternLength) {
      for (int i = 1; i < enemy.getPosLogSize() - patternLength; i++) {
        ...
        for (int j = 0; j < (patternLength - 2); j++) {
          diffDistance += Math.abs((enemy.getPosLogAt(i + j).distance(enemy.getPosLogAt(i + j + 1)))
              - (enemy.getPosLogAt(enemy.getPosLogSize() - (patternLength) + j)
                  .distance(enemy.getPosLogAt(enemy.getPosLogSize() - (patternLength - 1) + j))));

          diffTurnRate += Math.abs(MyUtils.angleBetween(enemy.getPosLogAt(i + j),
              enemy.getPosLogAt(i + j + 1), enemy.getPosLogAt(i + j - 1))
      ...
      if (enemy.getDistance() < 120) {
        ...
        try {
          while ((me.ownPos.distance(targetPos) > (20 - 3 * firepower) * i)
              && ((ptMinDiffEnd + 1 + i) < enemy.getPosLogSize())) {
            targetPosHelp = targetPos;
            targetPos = calcTargetPositionWithPatternMatching(me, targetPosHelp, targetPosPrev,
                enemy.getPosLogAt(ptMinDiffEnd - 1 + i), enemy.getPosLogAt(ptMinDiffEnd + i),
                enemy.getPosLogAt(ptMinDiffEnd + 1 + i));
          ...
          System.out
              .println("Index out of bound during patternMatching, targetPosition prediction");
          // fire = false;
        }
  ...
  private Point2D.Double calcTargetPositionWithPatternMatching(KnackOnOne me,
      Point2D.Double currentEnemyPos, Point2D.Double prevEnemyPos, Point2D.Double logPosPrev,
      Point2D.Double logPosCenter, Point2D.Double logPosNext) {
    ...
    Point2D.Double x0 = new Point2D.Double(currentEnemyPos.getX(), me.getBattleFieldHeight()); // angle
                                                                                               // between
                                                                                               // last
                                                                                               // position
                                                                                               // and
  ...
  private void fireAt(KnackOnOne me, double firepower, Point2D.Double p) {
    me.setTurnGunRightRadians(Utils
        .normalRelativeAngle(MyUtils.calcAbsBearing(me.ownPos, p) - me.getGunHeadingRadians()));
