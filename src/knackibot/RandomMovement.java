/**
 * This class implements random movement. Basis of the code and basic ideas were taken from
 * http://robowiki.net/wiki/RandomMovementBot/Code. Thanks to bot author PEZ.
 * 
...
public class RandomMovement implements MovementStrategy {
  ...
  RoundRectangle2D fieldRectangle(double margin) {
    return new RoundRectangle2D.Double(margin, margin, me.getBattleFieldWidth() - margin * 2,
        me.getBattleFieldHeight() - margin * 2, 75, 75);
  }
  ...
  void goTo(Point2D.Double destination) {
    double angle = Utils.normalRelativeAngle(
        MyUtils.calcAbsBearing(me.ownPos, destination) - me.getHeadingRadians());
  ...
  public void move(Enemy enemy, KnackOnOne me) {
    ...
    do {
      robotDestination = MyUtils.calcPoint(enemy.getCurrentPosition(),
          ...
          MyUtils.calcAbsBearing(enemy.getCurrentPosition(), me.ownPos)
              + localMovementLateralAngle);
      tries++;
